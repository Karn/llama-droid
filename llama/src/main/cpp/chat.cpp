            ...
            while (true) {
                ...
                if (n_dims == 1) {
                    ...
                    if (split_type == 0) {
                        if (tensor->ne[0] / n_parts != ne[0] || tensor->ne[1] != ne[1]) {
                            fprintf(stderr,
                                    "%s: tensor '%s' has wrong shape in model file: got [%d, %d], expected [%d, %d]\n",
                                    __func__, name.data(), tensor->ne[0] / n_parts, tensor->ne[1],
                                    ne[0], ne[1]);
                            return false;
                        }
                    } else {
                        if (tensor->ne[0] != ne[0] || tensor->ne[1] / n_parts != ne[1]) {
                            fprintf(stderr,
                                    "%s: tensor '%s' has wrong shape in model file: got [%d, %d], expected [%d, %d]\n",
                                    __func__, name.data(), tensor->ne[0], tensor->ne[1] / n_parts,
                                    ne[0], ne[1]);
                            return false;
                        }
                    }
                ...
                if (0) {
                    ...
                    fprintf(stderr, "%24s - [%5d, %5d], type = %6s, split = %d\n", name.data(),
                            ne[0], ne[1], ftype_str[ftype], split_type);
                }
                ...
                if (n_dims == 1 || n_parts == 1) {
                    ...
                    if (split_type == 0) {
                        ...
                        const size_t row_size = (tensor->ne[0] / ggml_blck_size(tensor->type)) *
                                                ggml_type_size(tensor->type);
                        assert(row_size == tensor->nb[1]);

                        for (int i1 = 0; i1 < ne[1]; ++i1) {
                            const size_t offset_row = i1 * row_size;
                            const size_t offset = offset_row +
...
bool llama_eval(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // feed-forward network
        {
            ...
            struct ggml_tensor *tmp = ggml_mul_mat(
                    ctx0,
                    model.layers[il].w3,
                    cur
            );
...
int infer(TokenCallback tokenCallback, int argc, char **argv) {
    ggml_time_init();
    ...
    params.top_p = 0.95f;
    params.n_ctx = 512;
    params.model = "/sdcard/alpaca7b.bin";
    ...
    __android_log_print(ANDROID_LOG_ERROR, "TAG", "Attempting to load model\n");
    ...
    static std::string prompt = "Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n";
    ...
    while (remaining_tokens > 0) {
        ...
        for (auto id: embd) {
            ...
            tokenCallback(vocab.id_to_token[id].c_str());
        }
    ...
    // report timing
    {
        ...
        __android_log_print(
                ANDROID_LOG_ERROR,
                "TAG",
                "mem per token = %8zu bytes\n    load time = %8.2f ms\n  sample time = %8.2f ms\n predict time = %8.2f ms / %.2f ms per token\n   total time = %8.2f ms\n",
                mem_per_token,
                t_load_us / 1000.0,
                t_sample_us / 1000.0f,
                t_predict_us / 1000.0f,
